...
function renderObjects() {
  ...
  updateSelectedFurnitureToolbar();
...
const GETTING_STARTED_KEY = "rowan-floor-planner-hide-getting-started-v1";

function closeGettingStarted() {
  byId("gettingStarted").hidden = true;

  if (byId("hideGettingStarted").checked) {
    window.localStorage.setItem(GETTING_STARTED_KEY, "true");
  } // closes persistent getting-started dismissal branch
} // closes closeGettingStarted

function showGettingStarted(forceShow = false) {
  const dismissed = window.localStorage.getItem(GETTING_STARTED_KEY) === "true";

  if (dismissed && !forceShow) {
    return;
  } // closes dismissed getting-started branch

  byId("gettingStarted").hidden = false;
} // closes showGettingStarted

async function showQuickFurniturePicker() {
  const list = byId("quickFurnitureList");
  ...
  try {
    ...
    records.sort(function (a, b) {
      return a.name.localeCompare(b.name);
    }); // closes quick furniture record sort

    if (!records.length) {
      const empty = document.createElement("p");

      empty.className = "small";
      empty.textContent = "No reusable furniture has been saved on this device yet.";
      list.appendChild(empty);
    } // closes empty quick furniture branch

    records.forEach(function (record) {
      const normalized = normalizeFurnitureLibraryData(record.data);
      ...
      row.className = "quick-furniture-item";
      name.textContent = record.name;
      dimensions.textContent = inchesToReadable(normalized.furniture.widthInches) + " by " + inchesToReadable(normalized.furniture.depthInches);
      ...
      addButton.addEventListener("click", async function () {
        byId("furnitureLibrarySelect").value = record.name;
        await addLibraryFurnitureToLayout();
        byId("quickFurniturePicker").hidden = true;
      }); // closes quick furniture add listener
    }); // closes quick furniture record loop

    byId("quickFurniturePicker").hidden = false;
  } catch (error) {
    announce("Could not open the furniture library: " + error.message);
  } // closes quick furniture picker try/catch
} // closes showQuickFurniturePicker

function openFurnitureEditorForNewItem() {
  byId("quickFurniturePicker").hidden = true;
  state.selectedId = null;
  byId("labelInput").value = "Furniture";
  byId("shapeInput").value = "rect";
  byId("colorInput").value = "#3b82f6";
  setFeetInches("widthFeet", "widthInches", 72);
  setFeetInches("depthFeet", "depthInches", 36);
  byId("rotationInput").value = "0";
  openControlPanel("furnitureEditorPanel");
  byId("labelInput").focus();
  announce("Enter the furniture name and dimensions, then choose Add edited furniture.");
} // closes openFurnitureEditorForNewItem

function editSelectedFurnitureFromToolbar() {
  const obj = objectById(state.selectedId);

  if (!obj) {
    announce("Select furniture first.");
    return;
  } // closes missing selected toolbar edit branch

  setEditorFurniture(obj);
  openControlPanel("furnitureEditorPanel");
  byId("labelInput").focus();
} // closes editSelectedFurnitureFromToolbar

function updateSelectedFurnitureToolbar() {
  const toolbar = byId("selectedFurnitureToolbar");
  const obj = objectById(state.selectedId);

  if (!toolbar) {
    return;
  } // closes missing toolbar branch

  if (!obj) {
    toolbar.hidden = true;
    return;
  } // closes no selected furniture toolbar branch

  toolbar.hidden = false;
  byId("selectedFurnitureToolbarName").textContent = obj.label;
  ...
  window.requestAnimationFrame(function () {
    ...
    const objectX = ratioToPixelX(obj.xRatio) / VIEWBOX_WIDTH * stageWidth;
    const objectY = ratioToPixelY(obj.yRatio) / VIEWBOX_HEIGHT * stageHeight;
    const objectWidth = dimensions.widthPx / VIEWBOX_WIDTH * stageWidth;
    const objectHeight = dimensions.heightPx / VIEWBOX_HEIGHT * stageHeight;
    const toolbarWidth = Math.max(1, toolbar.offsetWidth);
    const toolbarHeight = Math.max(1, toolbar.offsetHeight);
    const desiredLeft = objectX + objectWidth / 2 + 6;
    const desiredTop = objectY - objectHeight / 2 - toolbarHeight - 6;
    const left = clamp(desiredLeft, 4, Math.max(4, stageWidth - toolbarWidth - 4));
    const top = clamp(desiredTop, 4, Math.max(4, stageHeight - toolbarHeight - 4));

    toolbar.style.left = left + "px";
    toolbar.style.top = top + "px";
  }); // closes toolbar position frame callback
} // closes updateSelectedFurnitureToolbar

function wireV7UsabilityControls() {
  byId("quickAddFurniture").addEventListener("click", showQuickFurniturePicker);
  byId("gettingStartedHelp").addEventListener("click", function () {
    showGettingStarted(true);
  }); // closes help listener
  byId("closeGettingStarted").addEventListener("click", closeGettingStarted);
  byId("startPlanning").addEventListener("click", closeGettingStarted);
  byId("closeQuickFurniturePicker").addEventListener("click", function () {
    byId("quickFurniturePicker").hidden = true;
  }); // closes quick picker close listener
  byId("quickCreateFurniture").addEventListener("click", openFurnitureEditorForNewItem);
  byId("toolbarRotate").addEventListener("click", rotateObject);
  byId("toolbarEdit").addEventListener("click", editSelectedFurnitureFromToolbar);
  byId("toolbarDuplicate").addEventListener("click", duplicateObject);
  byId("toolbarDelete").addEventListener("click", deleteObject);
} // closes wireV7UsabilityControls
...
async function initialize() {
  ...
  announce("Planner v7.1 ready. Existing V6 layouts and furniture remain available on this device.");
} // closes initialize
